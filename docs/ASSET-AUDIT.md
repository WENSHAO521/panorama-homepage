...
into [`web/src/data/assets-manifest.json`](../web/src/data/assets-manifest.json).
The script only **reads** the legacy asset directories (`assets/`,
`Homepage Image/`, `QKFM/`, root favicons) — it never renames, edits,
or deletes an original file — and **copies** (never moves) files
classified `preserve` or `review` into the new `web/public/` structure
described in the project brief §8. Re-run any time to refresh the
manifest and re-sync `web/public/`.

Unlike the rest of the legacy repository, these asset directories were
deliberately **kept on disk** when the legacy HTML/CSS/JS was deleted
(per explicit instruction: "delete everything except images and
logos"), so this audit was run against the real files, not git history.
