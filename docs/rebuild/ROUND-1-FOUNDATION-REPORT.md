...
This report is a historical Round 1 snapshot. The later migration was
completed in the current `web/` application; retired legacy source files and
asset directories are no longer part of the release tree.
...
Recorded in
[`web/src/data/assets-manifest.json`](../../web/src/data/assets-manifest.json).
The original source files were read-only inputs during the audit; their
approved copies now live under `web/`, and the retired source tree has since
been removed.
